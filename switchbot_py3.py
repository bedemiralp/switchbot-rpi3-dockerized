...
def ensure_adapter_up(bt_interface: str):
    adapter = bt_interface or "hci0"
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            "Failed to bring bluetooth adapter {} up{}".format(
                adapter, ": {}".format(stderr) if stderr else ""
            )
        )
...
def connect(device: str, bt_interface: str, timeout: float):
    ensure_adapter_up(bt_interface)
...
class Scanner(object):
            ...
            with connect(device, bt_interface, timeout) as req:
                for chrc in req.discover_characteristics():
                    if chrc.get("uuid") == cls.service_uuid:
                        print(
                            " * Found Switchbot service on device {} handle {}".format(
                                device, chrc.get("value_handle")
                            )
                        )
                        return True
        except Exception:
    ...
    def scan(self):
        ensure_adapter_up(self.bt_interface)
...
class Driver(object):
    ...
    def get_rx_handle(self, req):
        for chrc in req.discover_characteristics():
            if chrc.get("uuid") == self.rx_uuid:
                return chrc.get("value_handle")

        raise RuntimeError(
            "SwitchBot RX characteristic {} not found on device {}".format(
                self.rx_uuid, self.device
            )
        )
    ...
    def run_command(self, command):
        with connect(self.device, self.bt_interface, self.timeout_secs) as req:
            print("Connected!")
            handle = self.get_rx_handle(req)
            print("Writing command to handle {}".format(handle))
            return req.write_by_handle(handle, self.commands[command])
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Name of the bluetooth adapter (default: system adapter)",
    )
    ...
    driver = Driver(
        device=bt_addr, bt_interface=opts.interface, timeout_secs=opts.connect_timeout
    )
